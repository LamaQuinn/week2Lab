...
// CODE HERE
let getCities = cities.filter((city)=>city.endsWith('i'))
...
// CODE HERE
let getFruit = fruits.forEach((name,index)=>{
   if(name.color==='red'){
     console.log(`The fruit with ${index} is an apple!`)
   }else if(name.color==='orange'){
    console.log(`The fruit with ${index} is an orange!`)
   }else{
...
// The fruit with index 0 is neither apple or orange.
// The fruit with 1 is an orange!
// The fruit with index 2 is neither apple or orange.
// The fruit with 3 is an apple!
////////// PROBLEM 5 //////////
...
// CODE HERE
let getPrice = foods
         .filter((name)=>name.tags.includes('rice'))
         .reduce((sum,name)=>sum+name.price,0)
